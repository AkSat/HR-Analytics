...
# set.seed(7)
# model_rfs <- train(is_promoted ~ .,data = trainSample[,-1], method = "rf")
# model_rfs$results$Accuracy
# prediction_rf <- predict(model_rfs, newdata = test1)
# solution_rf <- data.frame(employee_id=test1$employee_id,is_promoted = prediction_rf)
# write.csv(solution_rf,"submissions14_rf_scale_1.csv",row.names = FALSE)
# 
# set.seed(7)
# model_knn <- train(is_promoted ~ ., data = trainSample[,-1], method = "knn")
# model_knn$results$Accuracy
# 
# #Build model using SVM
# set.seed(7)
# model_svm <- train(is_promoted ~ ., data = trainSample[,-1], method = "svmRadial")
# model_svm$results$Accuracy
# 
# #Build model using Avg Neural Network
# set.seed(7)
# model_avNNet <- train(is_promoted ~ ., data = trainSample[,-1], method = "avNNet")
# model_avNNet$results$Accuracy
# 
# 
# sprediction_knn <- predict(model_knn, newdata = test1)
# sprediction_svm <- predict(model_svm, newdata = test1)
# sprediction_avNNet <- predict(model_avNNet, newdata = test1)
# 
# #Prepare dataset as per Kaggle submission file
# solution_knn <- data.frame(employee_id=test1$employee_id,is_promoted = sprediction_knn)
# solution_svm <- data.frame(employee_id=test1$employee_id,is_promoted = sprediction_svm)
# solution_avNNet <- data.frame(employee_id=test1$employee_id,is_promoted = sprediction_avNNet) #Takes long time to run 
# 
# write.csv(solution_knn,"submissions14_knn_scale_1.csv",row.names = FALSE)
# write.csv(solution_svm,"submissions14_svm_scale_1.csv",row.names = FALSE)
# write.csv(solution_avNNet,"submissions14_avNNet_scale_1.csv",row.names = FALSE)
# 
# 
# 
# # XGBOOST ===================================================
...
# str(trainSample)
# 
# l1 <- ifelse(trainSample$is_promoted == "1",1,0)
# 
# 
# 
# dtrain <- xgb.DMatrix(data = as.matrix(trainSample[,-c(1,9)]), label = l1)
# dtest <- xgb.DMatrix(data = as.matrix(test1[,-c(1,9)]), label = test1$is_promoted)
# 
...
# prediction <- as.numeric(ifelse(pred >= 0.5186667,1,0))
# solution_xgb <- data.frame(employee_id=test1$employee_id,is_promoted = prediction)
# write.csv(solution_xgb,"submissions14_xgb_scale.csv",row.names = FALSE)
